 ...
 **/
async function generatePollyUrl (ssml, voice, background_sound, polly_voice, sound_mix_result) {
    await copyFiles(); //has to invoke this function in order to copy sox / lame to /tmp/ to be able to execute them later. this has to happen every time because the tmp folder get purged every few minutes - todo: implement check if files exist and have the correct permissions +x
...
const LaunchHandler = {
  ...
  async handle(handlerInput) { //important make it ASYNC
    ...
    let background_sound = background_sfx; //defines background mp3 file found in the /audio/ folder in lambda function
    let sound_mix_result = "mixing-result_0.mp3"; //filename of soon to be mix result
    let polly_voice = "pollyvoice_tmp.mp3"; //tmp polly filename

    let pollyUrl = await generatePollyUrl("<speak>Another test with Polly voice Brian. Do you like my male voice? I certainly do like my fantastic voice! It's even greater with ocean waves in the background!</speak>", "Brian", background_sound, polly_voice, sound_mix_result);
    let easySpeakOutput = 'And here comes Brians polly voice with background sounds: <audio src="'+pollyUrl+'"/> Wow, that is so cool! ';
